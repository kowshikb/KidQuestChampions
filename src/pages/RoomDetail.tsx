...
const RoomDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(roomRef, (snapshot) => {
      ...
      showModal({
        ...
        message: "We had trouble connecting to the challenge room. Try again!",
        type: "error"
      });
      setLoading(false);
    });
  ...
  useEffect(() => {
    const fetchPlayerInfo = async () => {
      ...
      try {
        // Fetch player 1 info
        if (room.player1Id) {
          const player1Ref = doc(db, `${getBasePath()}/users/${room.player1Id}`);
          const player1Snap = await getDoc(player1Ref);
          
          if (player1Snap.exists()) {
            const data = player1Snap.data();
            setPlayer1Info({
              id: room.player1Id,
        ...
        if (room.player2Id) {
          const player2Ref = doc(db, `${getBasePath()}/users/${room.player2Id}`);
          const player2Snap = await getDoc(player2Ref);
          
          if (player2Snap.exists()) {
            const data = player2Snap.data();
            setPlayer2Info({
              id: room.player2Id,
              username: data.username,
              avatarUrl: data.avatarUrl
  ...
  const joinRoom = async () => {
    ...
    try {
      ...
      if (room.player1Id === currentUser.uid || room.player2Id) {
        return;
      }
      ...
      await updateDoc(roomRef, {
        messages: arrayUnion({
          senderId: 'system',
          text: `${userProfile?.username || 'A new player'} has joined the room!`,
          timestamp: Date.now()
        })
      });
  ...
  const challengePlayer = async () => {
    if (!roomId || !currentUser || !room || !selectedTheme || !selectedTask) return;
    
    try {
      playSound('challenge');
      
      // Determine the other player's ID
      const otherPlayerId = room.player1Id === currentUser.uid ? room.player2Id : room.player1Id;
      ...
      await updateDoc(roomRef, {
        messages: arrayUnion({
          senderId: 'system',
          text: `${userProfile?.username || 'A player'} has suggested a challenge!`,
          timestamp: Date.now()
        })
      });
  ...
  const acceptChallenge = async () => {
    ...
    try {
      ...
      await updateDoc(roomRef, {
        messages: arrayUnion({
          senderId: 'system',
          text: `${userProfile?.username || 'A player'} has accepted the challenge!`,
          timestamp: Date.now()
        })
      });
    } catch (error) {
  ...
  const rejectChallenge = async () => {
    ...
    try {
      ...
      await updateDoc(roomRef, {
        messages: arrayUnion({
          senderId: 'system',
          text: `${userProfile?.username || 'A player'} has declined the challenge.`,
          timestamp: Date.now()
        })
      });
  ...
  const completeChallenge = async () => {
    ...
    try {
      ...
      const task = theme?.tasks.find(t => t.id === room.currentChallenge?.taskId);
      ...
      await updateDoc(roomRef, {
        messages: arrayUnion({
          senderId: 'system',
          text: `${userProfile?.username || 'A player'} has completed the challenge first and won!`,
          timestamp: Date.now()
        })
      });
  ...
  const getCurrentChallengeDetails = () => {
    ...
    const challenger = room.currentChallenge.challengerId === room.player1Id ? player1Info : player2Info;
    const challenged = room.currentChallenge.challengedId === room.player1Id ? player1Info : player2Info;
  ...
  const isPlayerInRoom = () => {
    ...
    return room.player1Id === currentUser.uid || room.player2Id === currentUser.uid;
  };
    ...
    <div className="py-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-2xl font-bold text-purple-900">Challenge Room</h1>
            <div className="flex items-center text-purple-600">
              <span className="mr-2">Room ID:</span>
              <code className="bg-purple-50 px-2 py-1 rounded text-sm font-mono">{roomId}</code>
              <button
                onClick={copyRoomId}
            ...
            {!isPlayerInRoom() && (
              <div className="p-6 text-center">
                <div className="bg-yellow-50 border-2 border-yellow-200 p-4 rounded-xl mb-4">
                  ...
                  <p className="text-yellow-800">You are not a player in this room yet.</p>
                </div>
                <button
                  onClick={joinRoom}
                  className="btn-magic"
                  ...
                  {room?.messages && room.messages.length > 0 ? (
                    <div className="space-y-4">
                      {room.messages.map((msg, index) => {
                        ...
                        const sender = msg.senderId === player1Info?.id 
                          ? player1Info 
                          : msg.senderId === player2Info?.id 
                          ? player2Info 
                          : null;
                        ...
                        return (
                          ...
                          >
                            ...
                            <div>
                              {!isEmoji && (
                                <div className={`text-xs text-gray-500 mb-1 ${isCurrentUser ? 'text-right' : ''}`}>
                                  {isCurrentUser ? 'You' : sender?.username || 'User'}, {formatTime(msg.timestamp)}
                                </div>
                              )}
                    ...
                    <div className="h-full flex items-center justify-center">
                      <div className="text-center p-6">
                        <MessageCircle size={32} className="text-purple-300 mx-auto mb-2" />
        ...
        <div className="md:col-span-1">
          <div className="bg-white rounded-2xl shadow-md border-2 border-purple-100 overflow-hidden h-[600px] flex flex-col">
            ...
            <div className="p-4 flex-1 overflow-auto">
              {!isPlayerInRoom() ? (
                <div className="text-center p-6">
                  <p className="text-purple-600">Join the room to participate in challenges!</p>
                </div>
              ) : !areBothPlayersPresent() ? (
                <div className="text-center p-6">
                  <div className="bg-yellow-50 border-2 border-yellow-200 p-4 rounded-xl mb-4">
                    <AlertTriangle size={24} className="text-yellow-600 mx-auto mb-2" />
                    <p className="text-yellow-800">Waiting for another player to join...</p>
                  </div>
                  <p className="text-purple-600 mb-4">Share your room ID with a friend:</p>
                  <div className="flex justify-center">
                    <code className="bg-purple-50 px-3 py-2 rounded text-sm font-mono">{roomId}</code>
                    <button
                      onClick={copyRoomId}
                ...
                <div>
                  <div className={`p-4 rounded-xl mb-4 ${
                    challenge.status === 'pending' ? 'bg-yellow-50 border-2 border-yellow-200' :
                    challenge.status === 'accepted' ? 'bg-blue-50 border-2 border-blue-200' :
                    challenge.status === 'completed' ? 'bg-green-50 border-2 border-green-200' :
                    'bg-red-50 border-2 border-red-200'
                  }`}>
                    <h3 className="font-bold text-lg mb-2">{challenge.theme.name}</h3>
                    ...
                    <div className="flex justify-between items-center mb-3">
                      ...
                      <span className="font-medium">{challenge.challenger?.username || 'Unknown'}</span>
                    </div>
                    
                    <div className="flex justify-between items-center mb-3">
                      <span className="text-gray-600">Challenged:</span>
                      <span className="font-medium">{challenge.challenged?.username || 'Unknown'}</span>
                    </div>
                    ...
                    {challenge.status === 'completed' && challenge.winnerId && (
                      <div className="mt-4 p-3 bg-yellow-100 rounded-lg text-center">
                        <p className="font-bold text-yellow-800">
                          Winner: {challenge.winnerId === currentUser?.uid ? 'You!' : 
                                   challenge.winnerId === player1Info?.id ? player1Info.username :
                                   challenge.winnerId === player2Info?.id ? player2Info.username :
                                   'Unknown'}
                        </p>
                      </div>
                    )}
                  ...
                  {/* Challenge Actions */}
                  {challenge.status === 'pending' && challenge.challenged?.id === currentUser?.uid && (
                    <div className="flex space-x-2">
                      <button
                ...
                <div>
                  <p className="text-center text-purple-600 mb-6">Start a new challenge with your friend!</p>
                  
                  {isTaskSelectOpen ? (
                    <div className="space-y-4">
                      <h3 className="font-bold text-purple-900">Select a Quest:</h3>
                        ...
                        {themes.map((theme) => (
                          <div 
                            ...
                            className={`p-3 rounded-lg cursor-pointer transition-colors ${
                              selectedTheme === theme.id ? 'bg-purple-100 border-2 border-purple-300' : 'bg-gray-50 hover:bg-gray-100'
                            }`}
                            onClick={() => {
                              playSound('click');
                          ...
                          >
                            <div className="flex justify-between items-center">
                              <h4 className="font-bold text-purple-900">{theme.name}</h4>
                      ...
                      {selectedTheme && (
                        <div className="mt-4">
                          <h3 className="font-bold text-purple-900 mb-2">Select a Task:</h3>
                          <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                            {themes.find(t => t.id === selectedTheme)?.tasks.map((task) => (
                              <div 
                                key={task.id}
                                className={`p-3 rounded-lg cursor-pointer transition-colors ${
                                  selectedTask === task.id ? 'bg-purple-100 border-2 border-purple-300' : 'bg-gray-50 hover:bg-gray-100'
                                }`}
                                onClick={() => {
                                  playSound('click');
