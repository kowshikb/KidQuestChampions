...
const Dashboard: React.FC = () => {
  ...
  const recommendedQuest = themes.find(theme => 
    theme.tasks.some(task => 
      !userProfile?.completedTasks?.includes(task.id)
    )
  );
    ...
    >
      ...
      >
        <div className="bg-white rounded-xl p-4 shadow-md border-2 border-purple-100">
          ...
          <p className="text-2xl font-bold text-purple-900">{userProfile?.coins || 0}</p>
        </div>
        <div className="bg-white rounded-xl p-4 shadow-md border-2 border-purple-100">
          <p className="text-sm text-purple-500 mb-1">Quests Completed</p>
          <p className="text-2xl font-bold text-purple-900">{completedTasksCount}</p>
        </div>
        <div className="bg-white rounded-xl p-4 shadow-md border-2 border-purple-100">
          <p className="text-sm text-purple-500 mb-1">Friends</p>
          <p className="text-2xl font-bold text-purple-900">{userProfile?.friendsList?.length || 0}</p>
        </div>
        <div className="bg-white rounded-xl p-4 shadow-md border-2 border-purple-100">
          <p className="text-sm text-purple-500 mb-1">User ID</p>
          <p className="text-lg font-bold text-purple-900 truncate">{userProfile?.userId || '-'}</p>
        </div>
      </motion.div>
      ...
      {recommendedQuest && (
        ...
        >
          <h2 className="text-2xl font-bold text-purple-900 mb-4">Recommended Quest</h2>
          <div 
          ...
          >
            <div className="flex items-start">
              ...
              <div>
                <h3 className="text-xl font-bold mb-2">{recommendedQuest.name}</h3>
                <p className="opacity-90 mb-3">{recommendedQuest.description}</p>
